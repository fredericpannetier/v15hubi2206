        ...
        select_str = """
            WITH currency_rate as (%s)
             SELECT min(l.id) as id,
                    ...
                    , concat('M' , to_char(s.date_order, 'MM')) as order_month
                    , concat('T0' , to_char(s.date_order, 'Q')) as order_quarter
                    , concat('M' , to_char(s.di_sending_date, 'MM')) as sending_month
                    , concat('T0' , to_char(s.di_sending_date, 'Q')) as sending_quarter
    ...
    def _group_byC(self):
        ...
        group_by_str = """
            GROUP BY
                    ...
                    , dc.id, dc.name, s.di_sending_date,
                    s.invoice_status 
