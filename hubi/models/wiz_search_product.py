...
#    @api.multi
    def search_product(self):
        ...
        for p in self.env['sale.order'].sudo().browse(sale_order_ids):
            ...
            for product, date_start, date_end, unit_price, fishing_type, fishing_area, fishing_sub_area in ids:
                _logger.info("fishing_type : %s", fishing_type)
                _logger.info("fishing_area : %s", fishing_area)
                _logger.info("fishing_sub_area : %s", fishing_sub_area)
                ...
                _logger.info("price_vals : %s", price_vals)
...
#    @api.multi
    def update_sale(self):
        ...
        #price_list_ids = self.env['wiz.search.product.line'].browse(self._context.get('active_ids', [])).filtered(lambda p: p.qty_invoiced !=0  )
        price_list_ids = self.env['wiz.search.product.line'].search([ ('qty_invoiced', '!=', 0),   ])
        #_logger.info("Création pour les lignes : %s", price_list_ids)
        
        for price_list in price_list_ids:
            price_vals = {
                    'order_id': price_list.order_line_id,
                    'product_id': price_list.product_id.id,
                    'product_uom_qty':price_list.qty_invoiced,
                    'price_unit':price_list.price_unit,
                    'di_fishing_type_id':price_list.fishing_type_id.id,
                    'di_fishing_area_id':price_list.fishing_area_id.id,
                    'di_fishing_sub_area_id':price_list.fishing_sub_area_id.id,
                    
                     }

            #_logger.info("Création produit : %s", price_vals)
